...
function teleportBoatToPlayer()
    ...
    if boat then
        ...
        if basePart then
            basePart.CFrame = CFrame.new(playerHead.Position) + Vector3.new(0, -1, 0) 
        end
    end
end
    ...
    if Shake then
        ...
        shakeConnection = RunService.RenderStepped:Connect(function()
            if LocalPlayer.PlayerGui:FindFirstChild("shakeui") and LocalPlayer.PlayerGui.shakeui.safezone:WaitForChild("button") then
                local currentButton = LocalPlayer.PlayerGui:FindFirstChild("shakeui").safezone:WaitForChild("button")
...
ContextActionService:BindAction('ToggleTPBoat', ToggleTPBoat, false, Enum.KeyCode.G)
